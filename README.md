...
方式B（固定割り当て）を採用しています。Select は元仕様どおり LFO 一時停止として残し、方向キーと A 修飾で 3 パラメータを調整します。

| 元の操作子 | Game Boy割り当て | 実装挙動 |
|---|---|---|
| ゲート | A | 押している間だけ発音 |
| キル | B | 押している間は即ミュート。ゲート/Start長押しより優先 |
| LFO一時停止 | Select | 押している間 LFO の角度更新を停止 |
| 波形切替/手動発音 | Start | 約230ms未満の短押しで LFO 波形切替。長押し中は強制発音 |
| POT1 ピッチ | Up / Down | ベースピッチを増減。長押しで継続変化 |
| POT2 LFO深さ | A + Up / Down | LFO 深さを増減 |
| POT3 LFO速度 | Left / Right | LFO 速度を増減 |

注意: A は発音ゲートも兼ねるため、A+Up/Down で深さ調整中も発音します。
...
- B キル時はフェードせず即座に `NR12=0`、`NR14=0x80` で無音化します。
