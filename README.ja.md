...
| LFO波形表示 | UI更新ごとに文字で波形を再描画 | 固定Y座標テーブルをDEPTHに応じてスケールし、背景タイルで128x24pxの波形を生成 | 通常フレームのCPU負荷を削減し、表示を滑らかにする |
| 波形表示解像度 | 文字ベース表示 | 16x3背景タイル内のピクセル線 | 160x48の固定Y座標テーブルを128x24表示へ縮小して描画 |
